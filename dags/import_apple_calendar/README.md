...
6. **iCloud Drive folder** (PDF source, separate from Calendar)
   ...
   - Cloud-only files: Docker cannot download them. The task SSHs to this Mac (`host.docker.internal`) and runs `scripts/materialize_icloud.sh` only when a PDF is unreadable. The script waits 10s × 18, then exits. Requires **Remote Login** and `scripts/setup_host_ssh.sh`.
...
├── host_materialize.py        # task → SSH → host script
├── scripts/materialize_icloud.sh  # Mac host: download, wait 10s × 18, exit
...
If Docker cannot read an iCloud PDF (`Input/output error`, file still cloud-only), the same task SSHs to the Mac and runs `materialize_icloud.sh` (10s × 18). If it is still unreadable, that file is skipped and the next DAG run tries again. Already-imported files are not treated as deleted. There is no LaunchAgent.
