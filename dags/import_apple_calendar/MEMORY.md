...
- `source_config.json` is local (gitignored). Commit only `source_config.json.example`. Missing file → clear `FileNotFoundError` with copy instructions.

## TASTE

- `source_config.json` lists people/calendars (local copy from `.example`). Secrets stay in Airflow Variables; parse-time `ensure_all_project_variables()` creates missing keys and never overwrites.
